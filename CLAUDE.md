...
## Current State (Phase 8 complete + Phase 9 preview shipped; Phase 5 still pending)
...
- **Phase 4 workflows:** `service_requests` table + `generate_ticket_number(req_type)` SQL fn + per-type ticket prefixes (`POT-2026-0001`, `LGT-…`, etc.) backed by a Postgres sequence. The `chat` Edge Function carries an expanded system prompt that drives **two workflows** — 311/service-request intake (location → description → photo) and BTR renewal walkthrough (4 steps, one-at-a-time) — plus **sentiment-triggered handoff**. The LLM emits invisible sentinels (`[[ACTION:{...}]]`, `[[HANDOFF]]`) which a streaming-safe `SentinelFilter` strips from deltas server-side. On `ACTION` the function mints a ticket number, inserts into `service_requests`, and emits an `action` SSE event; on `HANDOFF` it emits a `handoff` event. ChatWidget renders a green ticket card and an amber handoff card. Photo handling went to **data URLs** (no bucket needed) — browser reads via FileReader, base64 rides on the chat request body for the current turn; server fills `service_requests.photo_url` and keeps base64 out of `messages.content` to avoid bloating LLM history.
- **Phase 8 admin dashboard:** `/admin` is gated by Supabase Auth via `@supabase/ssr` (cookie-based sessions, refreshed by `middleware.ts` on `/admin/*`). Two-layer gate: session JWT validated by `auth.getUser()` AND membership in `admin_users` allowlist (so a stray signup doesn't grant admin). Route group `(protected)` holds the gated tree; `/admin/login` lives outside it. Pages shipped: **Overview** (KPI cards + charts — see Phase 9 below), **FAQs** (topic list grouped by `faqs.topic_key` + side-by-side EN/ES editor that upserts both rows in one save), **Announcements** (CRUD with priority + active toggle, server actions; new `announcements` table EN/ES title+body, priority int, active bool), **Service requests** (filterable table over Phase 4 tickets — status / type / free-text on location, description, ticket #), **Conversations** (list + detail viewer showing full transcript, linked tickets, rating), **Sources** (Phase 7 placeholder showing planned crawl URLs for cityofdoral.com + doralpd.com). Migration adds `faqs.topic_key` text + partial unique index on `(topic_key, lang)` so the editor's upsert works cleanly; `scripts/seed-faqs.mjs` populates `topic_key` from the JSON `id`. Bootstrap admin: `admin@doral.demo` / `DoralDemo2026!` exists in `auth.users` AND in the `admin_users` allowlist (granted via Auth admin API + REST insert during Phase 8 setup). Public site has an **Admin** link with a lock icon in the utility bar so staff can reach `/admin/login` without typing the URL.
- **Phase 9 preview (on `/admin` overview):** Recharts dashboards layered on top of the KPI cards. 5 charts in a responsive 3-column grid — **conversations / day (14d)** area chart with navy gradient, **EN vs ES** donut (navy + gold), **rating distribution** color-graded bar (red 1-2★, gold 3★, green 4-5★), **LLM provider split (14d)** donut (proves the Groq → Gemini failover from RFP §3.2), **top 311 request types (14d)** horizontal bar. Per-chart CSV export. "Connect to Power BI" header pull-down with onboarding copy positioned for the procurement conversation (mock OData feed). Server fetchers run in parallel via `Promise.all`; recharts isolated in one client component (`DashboardCharts`) so the page stays an RSC. Phase 9 still leaves the dedicated `/analytics` page unbuilt.
- **Live admin dashboard via Supabase Realtime:** new migration adds `conversations`, `messages`, `service_requests`, `announcements`, `faqs` to the `supabase_realtime` publication. `LiveRefresh.tsx` is a reusable client component that subscribes to `postgres_changes` for the requested tables via the browser Supabase client, debounces (600ms), then calls `router.refresh()` — the surrounding server component re-runs and the dashboard updates in place without a full page reload. Small pulsing green dot + `updated Ns ago` indicator in each page header. Mounted on `/admin` (all 5 tables), `/admin/service-requests` (tickets only), and `/admin/conversations` (conversations + messages).
- **Light-theme fix:** `globals.css` used to honor `prefers-color-scheme: dark` and flip `--foreground` to near-white, which made typed text in chat + admin forms invisible on the all-light UI. Dropped the dark-mode override, set `color-scheme: light` on `:root`, and pinned `input/textarea/select` text to `#171717` with a slate-400 placeholder as a defensive default.
...
**Next — Phase 5:** Document parsing (extend paperclip to PDFs → parse with `unpdf` in the chat Edge Function → write to `uploaded_documents` table that's already migrated → bot answers against the uploaded doc; ship a sample Doral permit PDF for an instant demo).
...
- **Phase 8 — Admin dashboard** ✅ `@supabase/ssr` cookie auth + `middleware.ts` + two-layer gate (session + `admin_users` allowlist). Route group `(protected)` holds the gated pages. Shipped: Overview (4 KPIs), FAQs (topic list + side-by-side EN/ES editor that upserts both langs), Announcements (CRUD + priority/active), Service requests (filterable table), Conversations (list + transcript viewer + linked tickets + rating), Sources (Phase-7 placeholder). Bootstrap user `admin@doral.demo` / `DoralDemo2026!` exists in `auth.users`; needs one INSERT into `admin_users` to actually log in.
- **Phase 9 — Power BI-style analytics** 🟡 Partial: 5 Recharts charts (conv/day area, EN/ES donut, rating distribution, LLM provider donut, top request types bar) plus per-chart CSV export and a "Connect to Power BI" mock pulldown landed **on `/admin` overview** (via Realtime, the page updates live without manual refresh). Still to ship: a dedicated `/analytics` page (deeper drill-downs, date range pickers, intent classification chart).
